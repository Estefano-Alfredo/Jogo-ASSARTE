...
var erros_nivel_3_atual : int = 0
var tempo_restante_nivel_3_atual : int = 0
# --- FIM DA MODIFICAÇÃO ---
...
func resetar_dados_sessao_atual():
	...
	# --- INÍCIO DA MODIFICAÇÃO (Fase 3) ---
	erros_nivel_3_atual = 0
	tempo_restante_nivel_3_atual = 0
	# --- FIM DA MODIFICAÇÃO ---
	nome_do_jogador = ""
...
# --- INÍCIO DA NOVA FUNÇÃO (Fase 3) ---
func calcular_desempenho_fase_3(tempo_restante, erros):
	var tempo_max_f3 = 180.0 # Tempo que definimos na cena (3 minutos)
	var tempo_gasto = tempo_max_f3 - tempo_restante
	
	# Regra de tempo: 10% a cada 60s (igual Fase 1)
	var penalidade_tempo = floor(tempo_gasto / 60.0) * 10.0
...
func processar_e_salvar_relatorio_final():
	...
	# --- INÍCIO DA MODIFICAÇÃO (Fase 3) ---
	var relatorio_f3 = calcular_desempenho_fase_3(tempo_restante_nivel_3_atual, erros_nivel_3_atual)
	
	# Média agora é por 3
	var perc_geral = (relatorio_f1.percentual + relatorio_f2.percentual + relatorio_f3.percentual) / 3.0
	...
	var relatorio_final = {
		...
		# Adiciona dados da Fase 3
		"fase3_percentual": relatorio_f3.percentual,
		"fase3_estrelas": relatorio_f3.estrelas,
		"fase3_erros": erros_nivel_3_atual,
		"fase3_tempo_restante": tempo_restante_nivel_3_atual,
...
func exportar_para_csv(relatorio_dict):
	...
	# --- INÍCIO DA MODIFICAÇÃO (Fase 3) ---
	if not file_exists:
		# Adiciona colunas da Fase 3
		var header = "Nome,Data,F1_Percentual,F1_Estrelas,F1_Erros,F1_TempoRestante_s,F2_Percentual,F2_Estrelas,F2_Erros,F2_TempoRestante_s,F3_Percentual,F3_Estrelas,F3_Erros,F3_TempoRestante_s,Geral_Percentual,Geral_Estrelas\n"
		file.store_string(header)
	else:
		file.seek_end()
		
	# Adiciona dados da Fase 3
	var data_row = "%s,%s,%.2f,%d,%d,%d,%.2f,%d,%d,%d,%.2f,%d,%d,%d,%.2f,%d\n" % [
		relatorio_dict.nome,
		relatorio_dict.data,
		...
		# Dados Fase 3
		relatorio_dict.fase3_percentual,
		relatorio_dict.fase3_estrelas,
		relatorio_dict.fase3_erros,
		relatorio_dict.fase3_tempo_restante,
...
func _criar_arquivo_de_regras():
	...
	file.store_string("- Tempo Máximo: 180 segundos (3min 00s)\n")
	file.store_string("- Penalidade de Tempo: -10% de aproveitamento a cada 60 segundos gastos.\n")
	file.store_string("- Penalidade de Erro: -2% de aproveitamento por cada conexão errada.\n\n")
	# --- FIM DA MODIFICAÇÃO ---
